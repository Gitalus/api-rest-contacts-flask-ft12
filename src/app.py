...
def get_users():
    # [<User 1>, <User 2>, <User 3>] Lista de Objetos de Python
    users = User.query.all()
    # print(users)
    # [{"id": 1 }, {"id": 2}, {"id": 3}] Lista de Diccionarios de Python
...
def delete_users(id):
    ...
    if not user:
        return jsonify({"status": False, "msg": "User doesn't exists!"}), 404
    ...
    if request.method == 'GET':
        if contact_id is not None:
            contact = Contact.query.filter_by(
                user_id=user_id, id=contact_id).first()
            if not contact:
                return jsonify({"status": False, "msg": "Contact not found!"}), 404
